...
class Layout
{
    ...
    {
        // Ambil data ID dan token dari session aktif
        $id = $this->CI->session->userdata('user_id');
        $token = $this->CI->session->userdata('token');

        // Lakukan pencocokan token dan ID di database session
        $session_data = $this->CI->db->get_where('tbl_sessions', array('id_users' => $id, 'token' => $token))->row();
        if ($session_data) {
            // Pengecekan token masih aktif atau tidak
            $current_time = time();
            $token_expiry_time = $session_data->last_login;

            $session_lifetime = 3600;
            // print_r($token_expiry_time);
            // var_dump($current_time - $token_expiry_time   <= $session_lifetime);
            // die;

            if ($current_time - $token_expiry_time   <= $session_lifetime) {
                // Token masih aktif
                return true;
                $this->session->set_flashdata('message', 'Session Aktif');
            } else {
                // Token telah kadaluarsa, logout user
                $this->CI->session->sess_destroy();
                redirect('login'); // Redirect ke halaman login
            }
        } else {
            // Data session tidak ditemukan, redirect ke halaman login
            redirect('login');
        }
    }
}
