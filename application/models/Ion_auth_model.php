...
class Ion_auth_model extends CI_Model
{
	...
	public function remove_from_group($group_ids = FALSE, $user_id = FALSE)
	{
		...
		if (!empty($group_ids)) {
			...
			foreach ($group_ids as $group_id) {
				...
				if (
					isset($this->_cache_user_in_group[$user_id]) && isset($this->_cache_user_in_group[$user_id][$group_id])
				) {
	...
	public function set_session($user)
	{
		...
		$token_ = token_(time(), $user->{$this->identity_column}, $user->id);
		$session_data = array(
		...
		sessionsDb('tbl_sessions', $db_sess);
